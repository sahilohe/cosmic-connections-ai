...
export class PDFExporter {
  ...
  private static addFooter(pdf: jsPDF, pageWidth: number, pageHeight: number): void {
    ...
    pdf.text('Generated by AstroSoul', pageWidth / 2, pageHeight - 10, { align: 'center' });
    pdf.text('This analysis is for entertainment and self-reflection purposes only.', pageWidth / 2, pageHeight - 5, { align: 'center' });
  }
  
  /**
   * Export birth chart as horizontal PNG image with wide layout
   */
  static async exportBirthChartPNG(
    ...
    try {
      // Create a temporary container for the horizontal layout
      const tempContainer = document.createElement('div');
      tempContainer.style.position = 'absolute';
      tempContainer.style.left = '-9999px';
      tempContainer.style.top = '0';
             tempContainer.style.width = '1200px'; // Wide format
       tempContainer.style.height = '800px'; // Increased height for all data
      ...
      tempContainer.style.border = '2px solid #4a90e2';
      tempContainer.style.boxShadow = '0 0 20px rgba(74, 144, 226, 0.3)';
      
      // Add header section
      const headerSection = document.createElement('div');
      headerSection.style.width = '300px';
      ...
      headerSection.style.gap = '15px';
       ...
       title.textContent = 'AstroSoul Birth Chart Analysis';
      ...
      birthInfo.style.backgroundColor = 'rgba(74, 144, 226, 0.1)';
      birthInfo.style.padding = '15px';
      birthInfo.style.borderRadius = '8px';
      birthInfo.style.border = '1px solid rgba(74, 144, 226, 0.3)';
      
      const name = document.createElement('div');
      name.textContent = `Name: ${birthData.name}`;
      name.style.fontSize = '16px';
      name.style.fontWeight = 'bold';
      name.style.marginBottom = '10px';
      name.style.color = '#4a90e2';
      birthInfo.appendChild(name);
      
      const date = document.createElement('div');
      date.textContent = `Date: ${birthData.date}`;
      date.style.fontSize = '14px';
      date.style.marginBottom = '5px';
      birthInfo.appendChild(date);
      
      const time = document.createElement('div');
      time.textContent = `Time: ${birthData.time}`;
      time.style.fontSize = '14px';
      time.style.marginBottom = '5px';
      birthInfo.appendChild(time);
      
      const city = document.createElement('div');
      city.textContent = `Location: ${birthData.city}`;
      city.style.fontSize = '14px';
      city.style.marginBottom = '5px';
      birthInfo.appendChild(city);
      
      if (birthData.coordinates) {
        const coords = document.createElement('div');
        coords.textContent = `Coordinates: ${birthData.coordinates.lat.toFixed(4)}°N, ${birthData.coordinates.lng.toFixed(4)}°E`;
        coords.style.fontSize = '12px';
        coords.style.color = '#888';
        birthInfo.appendChild(coords);
      }
      
      headerSection.appendChild(birthInfo);
      
      // Get chart data from the existing element
      const chartElement = document.getElementById(chartElementId);
      if (chartElement) {
        // Clone the chart wheel
        const chartWheel = chartElement.querySelector('.aspect-square')?.cloneNode(true) as HTMLElement;
        if (chartWheel) {
          chartWheel.style.width = '300px';
          chartWheel.style.height = '300px';
          chartWheel.style.margin = '0 auto';
          headerSection.appendChild(chartWheel);
        }
      }
      ...
      dataSection.style.gap = '15px';
      
      // Get the original chart data
      const originalChartElement = document.getElementById(chartElementId);
      if (originalChartElement) {
        // Clone and style ALL chart data sections
        const chartDataElements = originalChartElement.querySelectorAll('.bg-muted\\/20');
        chartDataElements.forEach((element, index) => {
          const clonedElement = element.cloneNode(true) as HTMLElement;
          clonedElement.style.backgroundColor = 'rgba(74, 144, 226, 0.1)';
          clonedElement.style.border = '1px solid rgba(74, 144, 226, 0.3)';
          ...
          dataSection.appendChild(clonedElement);
        });
      }
      
      tempContainer.appendChild(dataSection);
      
             // Add to document temporarily
       document.body.appendChild(tempContainer);
       
       // Calculate actual height needed
       const actualHeight = Math.max(800, tempContainer.scrollHeight);
       tempContainer.style.height = `${actualHeight}px`;
       
       // Capture the horizontal layout
